...
func getPodDir(podUID types.UID) string {
	return filepath.Join(getPodsDir(), string(podUID))
}

func getPodsDir() string {
	return filepath.Join(getRootDir(), DefaultKubeletPodsDirName)
}
...
func SupportsSingleFileMapping() bool {
	// defaults to true for non-windows from kuberuntime_manager.go
	return true
}
